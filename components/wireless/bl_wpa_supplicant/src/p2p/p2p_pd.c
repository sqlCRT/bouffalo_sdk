			       ...
			       const u8 *data, size_t len, int rx_freq)
{
	...
	if (msg.wps_config_methods & WPS_CONFIG_PUSHBUTTON) {
		dev->flags |= P2P_DEV_PEER_GO_NEG_PBC_FALLBACK;
		p2p_dbg(p2p,
			"Peer " MACSTR
			" started Provision Discovery with pushbutton; remember GO Negotiation PBC fallback for later PIN-display interoperability",
			MAC2STR(sa));
	}
