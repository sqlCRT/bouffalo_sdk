...
static int p2p_peer_uses_pin_display_pbc_fallback(struct p2p_device *peer)
{
	return peer &&
		peer->wps_method == WPS_PIN_DISPLAY &&
		(peer->flags & P2P_DEV_PD_PEER_KEYPAD) &&
		(peer->flags & P2P_DEV_PEER_GO_NEG_PBC_FALLBACK);
}


static int p2p_peer_uses_pin_display_dir_override(struct p2p_device *peer)
{
	return peer &&
		peer->wps_method == WPS_PIN_DISPLAY &&
		(peer->flags & P2P_DEV_PD_PEER_DISPLAY);
}


static void p2p_note_pin_display_pbc_fallback(struct p2p_data *p2p,
					      ...
					      const char *context)
{
	...
	if (!(peer->flags & P2P_DEV_PEER_GO_NEG_PBC_FALLBACK)) {
		p2p_dbg(p2p,
			"Peer " MACSTR
			" is using pushbutton Device Password ID during %s after keypad provisioning; enabling PIN-display GO Negotiation fallback",
			MAC2STR(peer->info.p2p_device_addr), context);
	}
...
static void p2p_clear_pin_display_pbc_fallback(struct p2p_device *peer)
{
	if (!peer)
		return;
...
static u16 p2p_go_neg_pw_id(struct p2p_data *p2p, struct p2p_device *peer)
{
	u16 pw_id = p2p_wps_method_pw_id(peer ? peer->wps_method : WPS_NOT_READY);

	if (peer && peer->oob_pw_id)
		return peer->oob_pw_id;

	if (p2p_peer_uses_pin_display_pbc_fallback(peer)) {
		p2p_dbg(p2p,
			"Mirror pushbutton Device Password ID in GO Negotiation for peer "
			MACSTR " while keeping local WPS method in PIN display mode",
			MAC2STR(peer->info.p2p_device_addr));
					    ...
					    struct p2p_device *peer)
{
	...
	pw_id = p2p_go_neg_pw_id(p2p, peer);
	...
	if (dev && dev->flags & P2P_DEV_USER_REJECTED) {
		...
		status = P2P_SC_FAIL_REJECTED_BY_USER;
		   ...
		   (dev->wps_method == WPS_NOT_READY &&
		    (p2p->authorized_oob_dev_pw_id == 0 ||
		     ...
		     msg.dev_password_id))) {
		if (dev && msg.dev_password_id == DEV_PW_PUSHBUTTON)
			p2p_note_pin_display_pbc_fallback(
				p2p, dev, "early GO Negotiation Request");
		...
		case DEV_PW_REGISTRAR_SPECIFIED:
			...
			if (dev->wps_method == WPS_PIN_KEYPAD) {
				p2p_clear_pin_display_pbc_fallback(dev);
				break;
			}
			if (p2p_peer_uses_pin_display_dir_override(dev)) {
				p2p_dbg(p2p,
					"Accept registrar-specified Device Password ID from peer "
					MACSTR
					" even though local method is PIN display, because the peer previously requested the wrong PIN direction and local display mode is overriding it",
					MAC2STR(dev->info.p2p_device_addr));
				break;
			}
			if (dev->wps_method != WPS_PIN_KEYPAD) {
		...
		case DEV_PW_PUSHBUTTON:
			...
			if (p2p_peer_uses_pin_display_pbc_fallback(dev)) {
				p2p_note_pin_display_pbc_fallback(
					p2p, dev, "GO Negotiation Request");
				p2p_dbg(p2p,
					"Accept pushbutton Device Password ID from peer "
					MACSTR
					" because this peer already accepted keypad provisioning for a locally displayed PIN",
					MAC2STR(dev->info.p2p_device_addr));
				break;
			}
			if (dev->wps_method != WPS_PBC) {
			     ...
			     const u8 *data, size_t len, int rx_freq)
{
	...
	case DEV_PW_REGISTRAR_SPECIFIED:
		...
		if (p2p_peer_uses_pin_display_dir_override(dev)) {
			p2p_dbg(p2p,
				"Accept registrar-specified Device Password ID from peer "
				MACSTR
				" in GO Negotiation Response while local method stays in PIN display mode, because the peer previously tried to make us enter its PIN",
				MAC2STR(dev->info.p2p_device_addr));
			break;
		}
		if (dev->wps_method != WPS_PIN_KEYPAD) {
	...
	case DEV_PW_PUSHBUTTON:
		...
		if (p2p_peer_uses_pin_display_pbc_fallback(dev)) {
			p2p_note_pin_display_pbc_fallback(
				p2p, dev, "GO Negotiation Response");
			p2p_dbg(p2p,
				"Accept pushbutton Device Password ID from peer "
				MACSTR
				" in GO Negotiation Response because this peer previously switched to keypad provisioning for a displayed PIN",
				MAC2STR(dev->info.p2p_device_addr));
			break;
		}
		if (dev->wps_method != WPS_PBC) {
