...
/* Android devices may retry GO Negotiation Request a couple of seconds after
 * a missed GO Negotiation Response. Keep the authorization state alive long
 * enough to answer those retries with another successful response. */
#define P2P_GO_NEG_RESP_CONFIRM_TIMEOUT_USEC 5000000
			      ...
			      const struct p2p_message *msg)
{
	if (dev->info.wfd_subelems && msg->wfd_subelems) {
		if (wpabuf_len(dev->info.wfd_subelems) !=
		    wpabuf_len(msg->wfd_subelems))
			return 1;

		return os_memcmp(wpabuf_head(dev->info.wfd_subelems),
				 wpabuf_head(msg->wfd_subelems),
...
static int p2p_probe_req_ssid_match(const struct ieee802_11_elems *elems)
{
	if (elems->ssid == NULL)
		return 0;
	...
	if (elems->ssid_len >= P2P_WILDCARD_SSID_LEN &&
					 ...
					 u8 query_count)
{
	...
	if (p2p->go_neg_peer) {
		...
		if (p2p->go_neg_peer->wps_method == WPS_PIN_DISPLAY &&
		    (p2p->go_neg_peer->flags & P2P_DEV_PD_PEER_KEYPAD) &&
		    (p2p->go_neg_peer->flags &
		     P2P_DEV_PEER_GO_NEG_PBC_FALLBACK))
		...
		unsigned int rx_freq)
{
	...
	if (!p2p_probe_req_ssid_match(&elems)) {
		/* not using a P2P-compatible discovery SSID - ignore */
		p2p_dbg(p2p, "Probe Req not using P2P-compatible SSID - ignore it");
		return P2P_PREQ_NOT_PROCESSED;
	}
...
void p2p_set_go_intent(struct p2p_data *p2p, u8 go_intent)
{
	p2p->go_intent = go_intent;
}
...
static void p2p_go_neg_resp_cb(struct p2p_data *p2p, int success)
{
	...
	p2p_set_state(p2p, P2P_CONNECT);
	p2p_set_timeout(p2p, 0, P2P_GO_NEG_RESP_CONFIRM_TIMEOUT_USEC);
}
