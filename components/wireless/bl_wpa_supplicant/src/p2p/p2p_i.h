...
/*
 * Some phones accept a PIN-display Provision Discovery exchange, but still use
 * Push Button Device Password ID in GO Negotiation. Keep a per-peer quirk flag
 * so GO Negotiation can mirror that fallback without changing the actual WPS
 * role used later during provisioning.
 */
#define P2P_DEV_PEER_GO_NEG_PBC_FALLBACK BIT(23)
