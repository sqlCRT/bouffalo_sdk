...
-  **<state>**:
   ...
   - 位0：退出Wi-Fi 透传模式 时的提示信息。

     - 0：退出Wi-Fi 透传模式 时不打印提示信息。
     - 1：退出Wi-Fi 透传模式 时打印 ``+QUITT``。
   ...
   - 位2：Wi-Fi 透传模式、蓝牙低功耗SPP和蓝牙SPP的连接状态提示信息。
...
- 如果将位0设置为1，退出Wi-Fi 透传模式 时将提示 "+QUITT"。
...
AT+MINIDUMP
^^^^^^^^^^^
功能：以十六进制格式导出 minidump 分区。可执行 ``xxd -r -p hex.output crash.bin`` 将导出的十六进制数据转换为 ``crash.bin``，使用方法参考 ``examples/crash/README_v1.md``。
...
AT+EFUSE-W-HEX
^^^^^^^^^^^^^^
功能：以十六进制格式向 eFuse 写入数据。输入数据为十六进制字符串，转换为二进制后写入指定地址。数据按 4 字节对齐，写入后需要执行 AT+EFUSE-CFM 确认。
...
- **<nbytes>**: 写入字节数，范围：1 ~ 8192。
- **<address>**: 起始地址，十六进制格式，例如 ``"0x1c"``。
...
::
    ...
    // 发送十六进制字符串：01020304050607080910
...
AT+EFUSE-R-HEX
^^^^^^^^^^^^^^
功能：以十六进制格式读取 eFuse 数据。输出为读取到的二进制数据对应的十六进制字符串。
...
AT+FLASH-W-HEX
^^^^^^^^^^^^^^
功能：以十六进制格式向 Flash 写入数据。输入数据为十六进制字符串，转换为二进制后写入指定地址。
...
- **<nbytes>**: 写入字节数，范围：1 ~ 8192。
- **<address>**: 起始地址，十六进制格式，例如 ``"0x210000"``。
...
AT+FLASH-R-HEX
^^^^^^^^^^^^^^
功能：以十六进制格式读取 Flash 数据。输出为读取到的二进制数据对应的十六进制字符串。
...
-  **<ssid>**: 目标 AP 的 SSID，最大长度 32 字节 ASCII。若 SSID 中包含 ``,``, ``"`` 或 ``\\`` 等特殊字符，需要使用转义字符。
-  **<pwd>**: 密码，长度范围：8 ~ 63 字节 ASCII。

注意事项
""""""""

-  AT+SYSSTORE=1 时，凭据保存到 flash；否则仅保存在 RAM 中，重启后丢失。
-  最多可保存 5 组凭据。
-  如果 SSID 已存在，则更新对应密码。
-  当前仓库的 ``at_utils_crypto`` 实现为透传，不执行实际 AES 加密。
...
::
    ...
    // 添加 AP "MyWiFi" 的凭据
    AT+CWCREDADD="MyWiFi","12345678"

    // 添加包含特殊字符的 SSID
    AT+CWCREDADD="My\,WiFi","password123"


AT+CWCREDDEL
^^^^^^^^^^^^
功能：从凭据存储中删除 Wi-Fi 凭据。
...
-  该凭据会从内存缓存中删除。
-  如果该凭据已保存到 flash，也会同时从 flash 中删除。
-  如果指定 SSID 不存在，则返回 ERROR。
...
-  该命令只列出当前内存中的凭据。
-  AT+SYSSTORE=1 时添加的凭据会持久保存到 flash，重启后仍可加载。
-  AT+SYSSTORE=0 时添加的凭据仅保存在 RAM 中，重启后丢失。
-  输出不区分持久凭据和临时凭据。
...
-  **<ssid>**: 目标 AP 的 SSID，必须已通过 AT+CWCREDADD 预先保存。
-  **<reason_code>**: 含义同 AT+CWJAP 的错误原因码。

注意事项
""""""""

-  使用前需要启用 Station 模式（AT+CWMODE=1 或 3）。
-  SSID 必须已通过 AT+CWCREDADD 添加到凭据存储。
-  当前仓库的 ``at_utils_crypto`` 实现为透传，读取凭据时不会进行实际解密。
-  该命令不会修改 AT+SYSSTORE 配置，只使用已有凭据。
...
设置一个 TBTT 周期内 Wi-Fi 的工作时间，单位为毫秒。

**命令：**

::

    AT+CWCOEXDUTY=<active_ms>
...
-  **<active_ms>**: 一个 TBTT 周期（102.4 ms）内 Wi-Fi 的工作时间，范围：10 ~ 90。

   -  10: Wi-Fi 工作 10 ms，BLE 约 92 ms。
   -  50: Wi-Fi 工作 50 ms，BLE 约 52 ms。
   -  90: Wi-Fi 工作 90 ms，BLE 约 12 ms。

注意事项
""""""""

-  TBTT 周期固定为 102.4 ms（102400 us）。
-  该命令用于配置 Wi-Fi/BLE 共存使能后的时间片分配。
-  实际 Wi-Fi 占空比为 ``active_ms / 102.4``。
-  需要先通过 AT+CWCOEXEN=1 启用共存模式。
-  取值范围为 10 ~ 90 ms，以保证 Wi-Fi 和 BLE 都有可用时间片。
...
::
    ...
    // Wi-Fi 使用 50 ms，BLE 约使用 52 ms
    AT+CWCOEXDUTY=50

    OK

    // Wi-Fi 使用 80 ms，BLE 约使用 22 ms
    AT+CWCOEXDUTY=80
...
通过进入或退出 Wi-Fi 省电模式启用或禁用 Wi-Fi/BLE 共存。
...
-  **<enable>**:
   ...
   -  0: 禁用 Wi-Fi/BLE 共存模式，退出省电模式。
   -  1: 启用 Wi-Fi/BLE 共存模式，进入省电模式。

注意事项
""""""""

-  启用共存时会进入 Wi-Fi 省电模式，禁用共存时会退出 Wi-Fi 省电模式。
-  共存模式启用后，Wi-Fi 和 BLE 根据 AT+CWCOEXDUTY 配置的占空比共享 RF。
-  共存模式要求 Wi-Fi 工作在 Station 模式并已连接 AP。
-  为避免影响 Wi-Fi beacon 间隔，建议蓝牙连接间隔配置为非 100 ms 的整数倍。
...
AT+CSIDMASTART
^^^^^^^^^^^^^^
功能：启动 CSI（Channel State Information）DMA 采集。
...
配置一个 CSI DMA 缓冲区，用于采集目标设备的数据包信道估计信息。首次启动时会创建 CSI 工作任务，并通过 AT 口输出 ``+CSIDMA`` 报告。

注意事项
""""""""

-  只有设备处于 monitor/sniffer 模式，并收到源 MAC 与 ``<mac>`` 匹配的帧时，才会产生 CSI 数据。执行 AT+CSIDMASTART 后，还需要使用 `AT+CWMONITOR`_ 进入目标 AP 所在信道。
-  使用 CSI 采集时，建议 AT+CWMONITOR 的 ``<output>`` 参数设置为 0，以关闭原始十六进制 ``+CWMONITOR`` 输出。例如：``AT+CWMONITOR=1,11,,,0``。
-  ``<mac>`` 应设置为正在目标信道上发包的 AP/路由器 BSSID。
-  当前仅支持 802.11g legacy 模式。

**命令：**

::

    AT+CSIDMASTART=<mac>[,<buf>][,<mode>][,<ts>][,<interval>][,<ready>]
...
::
    ...
    +CSIDMA:<timestamp>, <port>, <mac>, <rssi>, <noise>, <cfo>, <agc>, <format>, <rate>, <mcs>, <beamformed>, <smoothing>, <nsts>, <nsc>, <csi data...>
    ...

参数
""""""""

-  **<mac>**: 目标设备 MAC 地址，格式为 ``"00:11:22:33:44:55"``，必填。
-  **<buf>**: 缓冲区索引，范围：0 ~ 5。
-  **<mode>**: CSI 模式，完整功能构建中生效，否则固定为 0。

   -  0: legacy LTF
   -  1: HT LTF
   -  2: auto

-  **<ts>**: 时间控制模式，完整功能构建中生效，否则固定为 2。
...
-  **<interval>**: 触发时间戳或间隔，单位：毫秒。
-  **<ready>**: ISR 后的缓冲区 ready 策略。

   -  0: 保持缓冲区 pending，直到重新标记 ready 前 DMA 不会覆盖该缓冲区。
   -  1: ISR 后总是标记缓冲区 ready（推荐）。
...
::
    ...
    // 1. 查询目标 AP 的 BSSID 和信道，例如 AT+CWJAP?
    //    +CWJAP:"ssid","b0:7f:b9:ff:73:6e",11,-45,0
    // 2. 针对 AP BSSID 启动采集
    AT+CSIDMASTART="b0:7f:b9:ff:73:6e",0

    OK

    // 3. 进入 AP 所在信道的 monitor 模式，output=0 关闭 +CWMONITOR 原始数据输出
    AT+CWMONITOR=1,11,,,0

    OK

    // 4. AP 发包时，AT 口输出 CSI 报告
    +CSIDMA:12345678, 0, b0:7f:b9:ff:73:6e, -45, -92, ...
    ...


AT+CSIDMASTOP
^^^^^^^^^^^^^
功能：停止 CSI DMA 采集并释放资源。
...
停止所有 CSI DMA 缓冲区。最后一个活动缓冲区停止后，CSI 工作任务会退出。
...
.. list-table:: 指令列表
   ...
   * - `AT+CIPDNS`_
     - 查询/设置DNS服务器信息
   * - `AT+MDNS`_
     - 启动/停止 mDNS 服务广播或查询局域网 mDNS 服务
   * - `AT+CIPSTATE`_
     - 获取TCP/UDP/SSL连接信息
   * - `AT+CIPRECVBUF`_
     - 查询/设置TCP/UDP/SSL接收缓冲区大小
   * - `AT+CIPSTART`_
     - 建立TCP连接、UDP传输或SSL连接
   * - `AT+CIPSTARTEX`_
     - 建立具有自动分配ID的TCP连接、UDP传输或SSL连接
   * - `AT+CIPTCPOPT`_
     - 查询/设置套接字选项
   * - `AT+CIPCLOSE`_
     - 关闭TCP/UDP/SSL连接
   * - `AT+CIPSEND`_
     - 在正常传输模式或Wi-Fi透传模式下发送数据
   * - `AT+CIPSENDL`_
     - 在正常传输模式下并行发送长数据
   * - `AT+CIPSENDLCFG`_
     - 设置AT+CIPSENDL命令的配置
   ...
   * - `AT+CIPSNTPINTV`_
     - 查询/设置SNTP时间同步间隔
   * - `AT+CIPRECONNINTV`_
     - 查询/设置Wi-Fi透传模式下TCP/UDP/SSL重新连接间隔
   * - `AT+CIPRECVMODE`_
     - 查询/设置套接字接收模式
   * - `AT+CIPRECVDATA`_
     - 在被动接收模式下获取套接字数据
   ...
   * - `AT+CIPSSLCSNI`_
     - 查询/设置SSL客户端服务器名称
   * - `AT+CIPSSLCALPN`_
     - 查询/设置SSL客户端应用层协议协商（ALPN）
   * - `AT+CIPSSLCPSK`_
     - 查询/设置字符串格式的SSL客户端预共享密钥（PSK）
   * - `AT+CIPSSLCPSKHEX`_
     - 查询/设置十六进制格式的SSL客户端预共享密钥（PSK）
   * - `AT+CIPSSLSCONF`_
     - 查询/设置 SSL 服务器配置
   * - `AT+CIPSSLSPSK`_
     - 查询/设置字符串格式的 SSL 服务器预共享密钥（PSK）
   * - `AT+CIPSSLSPSKHEX`_
     - 查询/设置十六进制格式的 SSL 服务器预共享密钥（PSK）
   * - `AT+IPERF`_
     - 测试BFLB独立模式下的吞吐量
   * - `AT+IPERFSTOP`_
     - 停止BFLB的吞吐量测试
...
AT+MDNS
^^^^^^^
功能：启动/停止 mDNS 服务广播，或查询局域网 mDNS 服务。

注意事项
""""""""

-  当前实现支持服务广播和 PTR 服务发现。
-  不支持直接 A/AAAA 查询、仅 SRV 查询和仅 TXT 查询。
-  该功能默认关闭，以减小默认固件体积。
...
::
    ...
    +MDNS:1,<"hostname">,<"service type">,<port>,<"instance">,<"proto">
...
::
    ...
    AT+MDNS=1,<"hostname">,<"service type">,<port>[,<"instance">][,<"proto">][,<txt number>][,<"key">,<"value">][...]
    AT+MDNS=0
...
- **<"hostname">**: 以 ``<hostname>.local`` 形式广播的主机名，最大长度 63 字节。
- **<"service type">**: 服务类型，例如 ``"_http"``，最大长度 63 字节。
- **<port>**: 服务端口，范围：1 ~ 65535。
- **<"instance">**: 可选的服务实例名，省略时使用 hostname。
- **<"proto">**: 可选的服务协议，支持 ``"_tcp"`` 和 ``"_udp"``，默认值为 ``"_tcp"``。
- **<txt number>**: 可选的 TXT 项数量，范围：1 ~ 5。每项由后续一组 ``<"key">,<"value">`` 配置。
...
查询局域网内指定 mDNS 服务的 PTR 记录。该命令使用 lwIP ``mdns_search_service()`` API，因此仅支持查询类型 ``0`` （PTR）。如果 mDNS 响应包中包含 SRV、TXT、A 或 AAAA 附加记录，AT 会在同一个结果块中解析并输出这些记录。

**命令：**

::

    AT+MDNS=2,0,<"service type">,<"proto">[,<timeout ms>][,<max results>]

**响应：**

AT 返回一条或多条记录，最后返回 OK：
...
- **<type>**: 查询类型，仅支持 ``0`` （PTR），其他类型返回 ERROR。
- **<"service type">**: 服务类型，例如 ``"_http"``，最大长度 63 字节。
- **<"proto">**: 服务协议，支持 ``"_tcp"`` 和 ``"_udp"``。
- **<timeout ms>**: 可选查询超时时间，单位：毫秒，范围：1000 ~ 180000，默认值：5000。
- **<max results>**: 可选最大返回结果块数量，范围：1 ~ 255，默认值：1。
...
-  **<remote port>**: 远程端口号。
-  **<keep_alive>**: 为套接字配置 `SO_KEEPALIVE <https://man7.org/linux/man-pages/man7/socket.7.html#SO_KEEPALIVE>`_ 选项，单位：秒。

   - 范围：[0,7200]。

     - 0: 禁用保活功能（默认）。
     - 1~7200: 启用保活功能。`TCP_KEEPIDLE <https://man7.org/linux/man-pages/man7/tcp.7.html#TCP_KEEPIDLE>`_ 值为 ``<keep_alive>``，`TCP_KEEPINTVL <https://man7.org/linux/man-pages/man7/tcp.7.html#TCP_KEEPINTVL>`_ 值为1，`TCP_KEEPCNT <https://man7.org/linux/man-pages/man7/tcp.7.html#TCP_KEEPCNT>`_ 值为3。
...
-  **<keepalive>**: 为套接字配置 `SO_KEEPALIVE <https://man7.org/linux/man-pages/man7/socket.7.html#SO_KEEPALIVE>`_ 选项，单位：秒。

   - 范围：[0,7200]。

     - 0: 禁用保活功能（默认）。
     - 1~7200: 启用保活功能。`TCP_KEEPIDLE <https://man7.org/linux/man-pages/man7/tcp.7.html#TCP_KEEPIDLE>`_ 值为 ``<keepalive>``，`TCP_KEEPINTVL <https://man7.org/linux/man-pages/man7/tcp.7.html#TCP_KEEPINTVL>`_ 值为1，`TCP_KEEPCNT <https://man7.org/linux/man-pages/man7/tcp.7.html#TCP_KEEPCNT>`_ 值为3。
...
::
    ...
    +CIPSSLSCONF:<auth_mode>,"<cert_name>","<key_name>","<ca_name>"
...
::
    ...
    // 单连接：(AT+CIPMUX=0)
    AT+CIPSSLSCONF=<auth_mode>[,"<cert_name>"][,"<key_name>"][,"<ca_name>"]

    // 多连接：(AT+CIPMUX=1)
    AT+CIPSSLSCONF=<linkid>,<auth_mode>[,"<cert_name>"][,"<key_name>"][,"<ca_name>"]
...
- **<auth_mode>**:
   ...
   - 0: 无认证，此时不需要证书和 CA。
   - 1: 服务器加载 CA 证书，用于验证客户端提供的证书。
   - 2: 服务器提供自身证书，供客户端验证。
   - 3: 双向认证。

- **<cert_name>**: 证书文件名。
- **<key_name>**: 私钥文件名。
- **<ca_name>**: 服务器用于验证客户端的 CA 证书文件名。
...
::
    ...
    // 单连接：(AT+CIPMUX=0)
    AT+CIPSSLSCONF=3,"server_1.crt","server_1.key","ca_1.crt"

    // 无认证：(AT+CIPMUX=1)
    AT+CIPSSLSCONF=0,0

    // 客户端认证：(AT+CIPMUX=1)
    AT+CIPSSLSCONF=0,2,"","","ca_1.crt"

    // 服务器认证：(AT+CIPMUX=1)
    AT+CIPSSLSCONF=0,1,"server_1.crt","server_1.key",""

    // 双向认证：(AT+CIPMUX=1)
    AT+CIPSSLSCONF=0,3,"server_1.crt","server_1.key","ca_1.crt"

注意事项
""""""""

- 若需配置立即生效，请在启动 SSL 服务器前执行此命令。


AT+CIPSSLSPSK
^^^^^^^^^^^^^
功能：查询/设置字符串格式的 SSL 服务器预共享密钥（PSK）。
...
- **<"psk">**: 字符串格式的预共享密钥，最大长度 32 字节。若 ``<"psk">`` 包含 ``\0`` 字符，请使用 `AT+CIPSSLSPSKHEX`_ 命令。
- **<"hint">**: 预共享密钥标识，最大长度 32 字节。

注意事项
""""""""

- 若需配置立即生效，请在建立 SSL 服务器连接前执行此命令。


AT+CIPSSLSPSKHEX
^^^^^^^^^^^^^^^^
功能：查询/设置十六进制格式的 SSL 服务器预共享密钥（PSK）。
...
查询 QCC74x 作为 SSL 服务器时的十六进制 PSK 配置。
...
- **<"psk">**: 十六进制格式的预共享密钥，最大长度 64 个十六进制字符（32 字节），每个字节由两个十六进制字符表示。
- **<"hint">**: 预共享密钥标识，最大长度 32 字节。
...
.. list-table:: 指令列表
   ...
   * - `AT+BLEGATTSSRVDEL`_
     - GATTS 删除服务
   * - `AT+BLEGATTSREGISTER`_
     - GATTS 服务注册/注销
   * - `AT+BLEGATTSCHAR`_
     - GATTS 发现特征值
   * - `AT+BLEGATTSCHARCRE`_
     - GATTS 创建特征值
   * - `AT+BLEGATTSNTFY`_
     - GATTS 特征值通知
   * - `AT+BLEGATTSIND`_
   ...
   * - `AT+BLEGATTCSRVDIS`_
     - GATTC 发现服务
   * - `AT+BLEGATTCCHARDIS`_
     - GATTC 发现特征值
   * - `AT+BLEGATTCWR`_
     - GATTC 特征值写入
   * - `AT+BLEGATTCRD`_
     - GATTC 特征值读取
   * - `AT+BLEGATTCSUBSCRIBE`_
     - GATTC 订阅服务器
   * - `AT+BLEGATTCUNSUBSCRIBE`_
     - GATTC 取消订阅服务器
   * - `AT+BLETXPWR`_
     - BLE 发射功率
   * - `AT+BLESECPARAM`_
   ...
   * - `AT+BLESECCANNEL`_
     - BLE 取消配对
   * - `AT+BLESECPASSKEYCONFIRM`_
     - BLE 配对密钥确认
   * - `AT+BLESECPAIRINGCONFIRM`_
     - BLE 配对确认
   * - `AT+BLESECPASSKEY`_
     - BLE 输入配对密钥
   * - `AT+BLESECGETLTKLIST`_
     - 查询 BLE 已配对设备列表
   * - `AT+BLESECUNPAIR`_
     - BLE 取消配对
   * - `AT+BLESECSTART`_
   ...
   * - `AT+BLEDISSET`_
     - BLE DIS 配置文件设置
   * - `AT+BLESETGAPAPPEARANCE`_
     - 设置 BLE GAP Appearance 值
...
::
    ...
    +BLESTATICADDR:"XX:XX:XX:XX:XX:XX"
    OK
...
设置 BLE 静态随机地址。该地址会持久保存，并用于后续 BLE 广播和连接等操作。

**命令：**

::

    AT+BLESTATICADDR=<static_random_addr>
...
- <static_random_addr>: 静态随机地址，格式为 ``"XX:XX:XX:XX:XX:XX"``。首字节最高两位必须为 ``0b11``，范围为 C0 ~ FF，用于标识静态随机地址。
...
::
    ...
    AT+BLESTATICADDR="C0:22:33:44:55:66"
    OK
    AT+BLESTATICADDR?
    +BLESTATICADDR:"C0:22:33:44:55:66"
    OK
...
AT+BLESETGAPAPPEARANCE
^^^^^^^^^^^^^^^^^^^^^^
功能：设置 Bluetooth GAP Appearance 值。
...
::
    ...
    AT+BLESETGAPAPPEARANCE=<parameter_value>
...
- **<parameter_value>**: GAP Appearance 值，范围：[0,65535]。
...
.. list-table:: 指令列表
   ...
   * - `AT+SLWKTIMER`_
     - 配置定时器作为唤醒源
   * - `AT+GET_AP_DTIM`_
     - 查询 STA 从 AP 获取到的 DTIM 值
   * - `AT+GET_TWT_SUPPORTED`_
     - 查询当前固件/平台是否支持 TWT
   * - `AT+SLWKDTIM`_
     - 配置 DTIM
   * - `AT+SLCLDTIM`_
   ...
   * - `AT+TWT_STATUS`_
     - 查询当前 TWT 会话状态
   * - `AT+LP_INTERVAL_GET`_
     - 查询 Station 省电监听间隔
   * - `AT+START_KEEPALIVE`_
     - 启动周期性 Null data 保活
   * - `AT+STOP_KEEPALIVE`_
     - 停止周期性 Null data 保活
   * - `AT+GET_CLOCK`_
...
^^^^^^^^^^^^^^^^^^^^
功能：查询当前固件或平台是否支持 TWT（Target Wake Time）。
...
AT+TWT_STATUS
^^^^^^^^^^^^^
功能：查询当前 TWT（Target Wake Time）会话状态。
...
- **<flow_id>**: TWT 流 ID。
- **<flow_type>**: TWT 流类型，0 表示公告式，1 表示非公告式。
- **<flow_type_str>**: 流类型字符串，``"ANNOUNCED"`` 或 ``"UNANNOUNCED"``。
- **<wake_int_exp>**: 唤醒间隔指数。
- **<min_wake_dur>**: 最小 TWT 唤醒持续时间。
- **<wake_int_mantissa>**: 唤醒间隔尾数。
...
AT+LP_INTERVAL_GET
^^^^^^^^^^^^^^^^^^
功能：查询 Station 省电模式下的 listen interval 值。
...
- **<interval>**: listen interval 值，表示 STA 在省电模式下唤醒监听 beacon 的间隔。


AT+START_KEEPALIVE
^^^^^^^^^^^^^^^^^^
功能：启动周期性发送 Null data 帧，以维持与 AP 的连接。
...
- **<period>**: Null data 帧发送间隔，单位：秒。

注意事项
""""""""

-  该命令会创建定时器，周期性向已连接的 AP 发送 Null data 帧，避免空闲期间连接超时。
-  该功能仅在已连接 AP 时生效。
...
-  该命令停止由 AT+START_KEEPALIVE 创建的定时器。
-  如果当前没有活动的 keepalive 定时器，该命令仍返回 OK。
