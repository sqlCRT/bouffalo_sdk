// Custom (non-SquareLine) animations for the UICODE800 overlay.
//
...
//
//   - the four side icons (wifi, bluetooth, speed, comment) light up GREEN one
//     after another, each staying green for 1 s, then return to their original
//     look; the cycle repeats forever.
//   - the vertical slider sweeps from its minimum to its maximum value and back,
//     forever.
//
// This is the NATIVE landscape 800x480 DPI panel, so unlike UICODE480/UICODE720
// none of the widgets are rotated; the icon recolor and slider sweep below are
// orientation-independent and need no change for that.
...
/* The four icons to cycle through, in order (right-side column on the panel). */
 ...
 * which dominates CPU on a big panel and also makes the slider strobe/ghost in
 * the DIRECT double buffer when it moves fast.
 *
...
void ui_anim_start(void)
{
    ...
    /* Sweep the single slider min->max->min, forever, via a low-rate timer.
     * (The marquee label scrolls on its own, animated by LVGL.) */
